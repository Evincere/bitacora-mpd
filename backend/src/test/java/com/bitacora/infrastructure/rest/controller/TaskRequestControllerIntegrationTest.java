...
class TaskRequestControllerIntegrationTest {
    ...
    void shouldCreateTaskRequest() throws Exception {
        ...
        when(createTaskRequestUseCase.createDraft(anyString(), anyString(), any(), any(), any(), any(), anyLong()))
        ...
        mockMvc.perform(post("/api/task-requests")
                .with(user(UserPrincipal.create(1L, "user", "password", Collections.singletonList("ROLE_SOLICITANTE"))))
    ...
    void shouldGetTaskRequestById() throws Exception {
        ...
        when(taskRequestWorkflowService.findById(taskRequestId)).thenReturn(Optional.of(taskRequest));
    ...
    void shouldReturn404WhenTaskRequestNotFound() throws Exception {
        ...
        when(taskRequestWorkflowService.findById(taskRequestId)).thenReturn(Optional.empty());
    ...
    void shouldGetMyTaskRequests() throws Exception {
        ...
        when(taskRequestWorkflowService.findByRequesterId(eq(requesterId), eq(page), eq(size))).thenReturn(taskRequests);
        ...
        mockMvc.perform(get("/api/task-requests/my-requests")
                .with(user(UserPrincipal.create(requesterId, "user", "password", Collections.singletonList("ROLE_SOLICITANTE")))))
                .andExpect(status().isOk())
    ...
    void shouldAssignTaskRequest() throws Exception {
        ...
        mockMvc.perform(post("/api/task-requests/{id}/assign", taskRequestId)
                .with(user(UserPrincipal.create(assignerId, "user", "password", Collections.singletonList("ROLE_ASIGNADOR")))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(1)))
    ...
    void shouldCompleteTaskRequest() throws Exception {
        ...
        when(taskRequestWorkflowService.complete(taskRequestId)).thenReturn(completedTaskRequest);
        ...
        verify(taskRequestWorkflowService).complete(taskRequestId);
