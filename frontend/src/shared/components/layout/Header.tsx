...
const UserMenuItem = styled.div<UserMenuItemProps>`
  padding: 10px 16px;
  display: flex;
  align-items: center;
  cursor: pointer;
...
const SubmenuItem = styled.div`
  ...
  &.active {
    background-color: ${({ theme }) => theme.backgroundAlt || theme.backgroundHover};
    color: ${({ theme }) => theme.primary};
...
/**
 * Interfaz simplificada de usuario para el componente Header.
 *
 * @interface User
 ...
 * Muestra la barra de búsqueda, notificaciones, y el menú de usuario.
 *
 * @component
 * @example
 * ```tsx
...
const Header: React.FC = () => {
   ...
   * Cierra el menú de usuario, muestra una notificación y llama a la función de logout.
   */
  ...
  /**
   * Maneja los clics fuera del menú de usuario para cerrarlo.
   *
   ...
   * Actualiza el estado global y muestra una notificación.
   */
  ...
  /**
   * Alterna la visibilidad de un submenú específico.
   *
   * @param {string} submenu - Identificador del submenú a alternar
   */
  ...
  /**
   * Establece el tiempo de inactividad antes del cierre automático de sesión.
   *
   * @param {number} minutes - Tiempo en minutos antes del cierre automático
   */
  const handleSetInactivityTime = (minutes: number) => {
    // Guardar en localStorage
    localStorage.setItem('inactivity_timeout', (minutes * 60 * 1000).toString());
