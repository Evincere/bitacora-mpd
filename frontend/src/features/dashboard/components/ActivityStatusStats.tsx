...
import { statusColors } from '@/shared/styles';
...
const ActivityStatusStats: React.FC = () => {
  const { data, isLoading, isError, error } = useActivityStatsByStatus();
  ...
  const chartData = React.useMemo(() => {
    ...
    const colors = data.map(item => {
      // Obtener color del estado de actividad
      const activityStatus = item.status.toUpperCase();
      return statusColors[activityStatus]?.background || 'rgba(204, 204, 204, 0.15)';
    });
...
export default ActivityStatusStats;
